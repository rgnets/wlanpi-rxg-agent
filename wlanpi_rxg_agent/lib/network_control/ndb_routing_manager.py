...
class RoutingManager:
    ...
    async def startup_cleanup(self, managed_interfaces: Set[str]):
        ...
        with NDB() as ndb:
            ...
            for rule in ndb.rules:
                if (hasattr(rule, 'table') and 
                    self.base_table_id <= rule.table < self.base_table_id + 1000):
                    rules_to_remove.append(rule)
            
            self.logger.info(f"Found {len(rules_to_remove)} rules in managed table range")
            
            for rule in rules_to_remove:
                try:
                    rule.remove().commit()
                    self.logger.debug(f"Removed rule: table={rule.table}, priority={rule.priority}")
    ...
    def _flush_table_ndb(self, ndb: NDB, table_id: int):
        ...
        if routes_to_remove or rules_to_remove:
            self.logger.info(f"Flushed routing table {table_id} (removed {len(routes_to_remove)} routes and {len(rules_to_remove)} rules)")
    ...
    def _setup_interface_routing_ndb(
        ...
        try:
            with NDB() as ndb:
                ...
                self.logger.debug(f"Added subnet route: {network} via {interface.name} (table {table_id})")
                ...
                if gateway:
                    ...
                    try:
                        ...
                        self.logger.debug(f"Adding main table route: dst=default, gateway={gateway}, metric={main_metric}, oif={interface.index}, table=254")
                        ...
                        self.logger.info(f"Added gateway {gateway} to main table for {interface.name} with metric {main_metric}")
                        
                    except Exception as e:
                        self.logger.warning(f"Failed to add gateway to main table for {interface.name}: {e}")
                ...
                self.logger.info(f"Successfully configured routing for {interface.name} (table {table_id})")
                return True

        except Exception as e:
            self.logger.error(f"Error setting up interface routing for {interface.name}: {e}")
            return False
            ...
            with NDB() as ndb:
                ...
                if not src_ip:
                    for addr in ndb.addresses:
                        if (addr.index == interface_index and 
                            hasattr(addr, 'address') and 
                            not str(addr.address).startswith('fe80')):  # Skip link-local
                ...
                self.logger.debug(f"Added host route: {host_ip}/32 via {interface_name} (table {target_table}){src_info}")
                return True

        except Exception as e:
            self.logger.error(f"Error adding host route for {host_ip} via {interface_name}: {e}")
            return False
    ...
    async def remove_host_route(self, host_ip: str, interface_name: str) -> bool:
        ...
        try:
            with NDB() as ndb:
                ...
                for route in ndb.routes:
                    if (route.table == table_id and 
                        route.dst == f"{host_ip}/32"):
                        route_to_remove = route
                        break
                
                if route_to_remove:
                    route_to_remove.remove().commit()
                    self.logger.debug(f"Removed host route: {host_ip}/32 from {interface_name}")
                    return True
                else:
                    self.logger.warning(f"Host route {host_ip}/32 not found for {interface_name}")
                    return False

        except Exception as e:
            self.logger.error(f"Error removing host route for {host_ip} via {interface_name}: {e}")
            return False
        ...
        try:
            with NDB() as ndb:
                ...
                if table_id:
                    ...
                    self.logger.info(f"Successfully removed routing for {interface_name}")
                    return True
                else:
                    self.logger.warning(f"No routing configuration found for {interface_name}")
                    return False
        except Exception as e:
                ...
                if interface_name:
                    ...
                    return {
                        interface_name: {
                            ...
                            "main_table_routes": len(self.main_table_routes.get(interface_name, []))
                        }
                    }
                else:
            ...
            with NDB() as ndb:
                ...
                for route_info in routes_info:
                    try:
                        ...
                        for route in ndb.routes:
                            if (route.table == 254 and 
                                route.dst == route_info["dst"] and
                                route.gateway == route_info["gateway"] and
                                route.oif == route_info["oif"]):
                                route.remove().commit()
                                self.logger.debug(f"Removed main table route: {route_info}")
                                break
                    except Exception as e:
                        self.logger.warning(f"Failed to remove main table route {route_info}: {e}")
                
                # Clear the tracking
                self.main_table_routes.pop(interface_name, None)
                return True
        except Exception as e:
            self.logger.error(f"Error removing main table routes for {interface_name}: {e}")
            return False
