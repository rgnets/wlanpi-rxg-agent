    ...
    def _create_monitoring_interfaces(self):
        ...
        for interface_name in wireless_interfaces:
            try:
                ...
                self.logger.info(f"Successfully created monitoring interface for {interface_name}")
            except Exception as e:
                self.logger.warning(f"Failed to create monitoring interface for {interface_name}: {e}")
