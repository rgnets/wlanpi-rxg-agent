    ...
    async def execute(self, callee: str, post_connect: Optional[str] = None, call_timeout: Optional[int] = None,
        ...
        async with MdkBareSIP(gateway=self._gateway, user=self._user, pwd=self._password, debug=self._debug, extra_login_args=self._extra_login_args, config_path=self._config_path, interface=self._interface) as bs:
            ...
            async def call_established(bs_instance: MdkBareSIP, *_, **__):
                ...
                if not call_timeout:
                    bs_instance.hang()
                if not (test_in_progress.done() or test_in_progress.cancelled()): test_in_progress.set_result(True)
            ...
            def handle_call_ended_abnormally(bs_instance: CustomBaresipy, *_, **__):
                ...
                if not (call_in_progress.done() or call_in_progress.cancelled()): call_in_progress.set_result(False)
            ...
            def handle_call_ended_normally(bs_instance: CustomBaresipy, *_, **__):
                ...
                if not (call_in_progress.done() or call_in_progress.cancelled()): call_in_progress.set_result(True)
            ...
            try:
                ...
                bs.hang()
                if not (call_in_progress.done() or call_in_progress.cancelled()): call_in_progress.cancel(msg="Test timeout reached.")
                if not (test_in_progress.done() or test_in_progress.cancelled()): test_in_progress.cancel(msg="Test timeout reached.")
            ...
            try:
                ...
                self.logger.warning(f"No summary received after {call_timeout} seconds.")
