...
from cryptography.hazmat.backends.openssl.x509 import _CertificateSigningRequest, _Certificate
from cryptography.hazmat.primitives import serialization, hashes
from cryptography.hazmat.primitives.asymmetric import rsa
from cryptography.hazmat.backends import default_backend
from cryptography.hazmat.primitives.serialization import load_pem_private_key
from cryptography.x509 import NameOID, load_pem_x509_csr, load_pem_x509_certificate
...
class CertificateTool:
    ...
    def save_cert_from_pem(self, cert:str, cert_file:Optional[str]=None):
        # This seems a silly way to do it, but it will serve as a validator so we don't store garbage.
    ...
    def get_csr(self, node_name:str ):
        ...
        if os.path.exists(self.csr_file):
            ...
            builder = builder.add_extension(
                x509.KeyUsage(digital_signature=True, content_commitment=False, data_encipherment=False, key_agreement=False, crl_sign=True, key_encipherment=True, encipher_only=False, decipher_only=False, key_cert_sign=False ), critical=True,
            )
            # builder = builder.add_attribute(
            #     AttributeOID.CHALLENGE_PASSWORD, b"changeit"
            # )
            request = builder.sign(
                self.get_key(), hashes.SHA256()
            )
