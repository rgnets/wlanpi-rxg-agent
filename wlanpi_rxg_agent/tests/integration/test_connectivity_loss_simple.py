        ...
        try:
            ...
            try:
                ...
                if len(connectivity_lost_events) > 0:
                    logger.info(f"✓ SUCCESS: Received {len(connectivity_lost_events)} ConnectivityLost events")
                    event = connectivity_lost_events[0]
                    logger.info(f"  Interface: {event.interface.name}")
                    logger.info(f"  State reset: IP={event.interface.ip_address}, Gateway={event.interface.gateway}")
                    return True
                else:
            ...
            if hasattr(message_bus, '_handlers') and NetworkMessages.ConnectivityLost in message_bus._handlers:
                if hasattr(message_bus._handlers[NetworkMessages.ConnectivityLost], 'remove'):
                    message_bus._handlers[NetworkMessages.ConnectivityLost].remove(track_event)
                elif hasattr(message_bus._handlers[NetworkMessages.ConnectivityLost], 'discard'):
                    message_bus._handlers[NetworkMessages.ConnectivityLost].discard(track_event)
