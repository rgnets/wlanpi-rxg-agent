...
async def check_route_exists(
    dst: str, table: int = 254, src: Optional[str] = None
) -> bool:
    ...
    try:
        ...
        for line in result.stdout.split("\n"):
            if line.strip().startswith("default"):
                ...
                routes.append({"line": line.strip(), "metric": metric or 0})
...
async def test_host_route_with_source_ip(routing_manager, available_interfaces):
    ...
    success = await routing_manager.add_host_route(
        host_ip=test_host, interface_name=interface_name
    )
    ...
    test_host_2 = "8.8.4.4"
    success = await routing_manager.add_host_route(
        host_ip=test_host_2, interface_name=interface_name, src_ip=interface_ip
    )
...
async def test_main_table_gateway_management(routing_manager, available_interfaces):
    ...
    success = await routing_manager._setup_interface_routing(
        interface_info, routing_manager._get_table_id(interface_name), test_gateway
    )
    ...
    assert (
        interface_name in routing_manager.main_table_routes
    ...
    assert len(routes_after) > initial_count, "No new route added to main table"
    ...
    assert (
        len(routes_final) == initial_count
...
async def test_main_table_host_route_management(routing_manager, available_interfaces):
    ...
    route_exists = await check_route_exists(
        f"{test_host}/32", table=254, src=interface_ip
    )
    ...
    assert (
        not route_exists_after
    ), f"Host route {test_host}/32 still exists after cleanup"
...
async def test_comprehensive_cleanup(routing_manager, available_interfaces):
    ...
    assert (
        len(cleanup_main_routes) == initial_count
