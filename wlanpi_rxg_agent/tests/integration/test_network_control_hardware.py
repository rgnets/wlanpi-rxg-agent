...
async def test_netlink_monitor_real_interfaces(available_wireless_interfaces):
    ...
    async def event_callback(event_type, msg, interface_info):
        ...
        logger.info(
            f"Received {event_type} for {interface_info.name if interface_info else 'unknown'}"
        )
    ...
    try:
        ...
        logger.info(
            f"Monitor successfully started with {len(available_wireless_interfaces)} interfaces"
        )
...
async def test_network_manager_with_real_interfaces(available_wireless_interfaces):
    ...
    try:
        ...
        assert (
            interface_name in manager.managed_interfaces
        ), f"Interface {interface_name} should be managed"

        interface_info = manager.managed_interfaces[interface_name]
        logger.info(
            f"Discovered interface: {interface_info.name}, state: {interface_info.state}"
        )
...
@pytest.mark.hardware
@pytest.mark.skipif(
    not Path("/sys/class/net/wlan0").exists(), reason="wlan0 interface not available"
)
@pytest.mark.asyncio
async def test_interface_info_gathering():
    ...
    if interface_info:
        ...
        logger.info(
            f"Successfully gathered info for wlan0: state={interface_info.state}"
        )
    else:
...
async def test_extended_monitoring(available_wireless_interfaces):
    ...
    async def event_callback(event_type, msg, interface_info):
        ...
        logger.info(
            f"Event: {event_type} for {interface_info.name if interface_info else 'unknown'}"
        )
    ...
    logger.info(
        f"Extended monitoring completed. Captured {len(events_received)} events:"
    )
