...
async def test_simple_connectivity_loss():
    ...
    try:
        ...
        try:
            ...
            assert (
                len(connectivity_lost_events) > 0
            ), "Expected ConnectivityLost event to be emitted"

            event = connectivity_lost_events[0]
            assert (
                event.interface.name == "wlan0"
            ), f"Expected event for wlan0, got {event.interface.name}"
            assert (
                event.interface.ip_address is None
            ), "IP address should be reset to None"
            assert event.interface.gateway is None, "Gateway should be reset to None"
            assert (
                event.interface.has_dhcp_lease is False
            ), "DHCP lease flag should be reset to False"
        ...
        try:
            ...
            try:
                ...
                connecting_event = WiFiMessages.WpaSupplicantStateChanged(
                    interface="wlan0", state="completed"
                )
                ...
                assert len(connectivity_lost_events) == len(
                    disconnecting_states
                ), f"Expected {len(disconnecting_states)} events, got {len(connectivity_lost_events)}"

                logger.info(
                    f"✓ SUCCESS: State change detection working correctly ({len(connectivity_lost_events)} events)"
                )
